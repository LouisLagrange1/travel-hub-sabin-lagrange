...
export async function GET(req: NextRequest) {
  ...
  console.log(
    `Requête GET: Recherche des offres de ${from} à ${to} avec une limite de ${limit}`
  );
  ...
  try {
    ...
    console.log(
      "Données non trouvées dans le cache, récupération depuis MongoDB..."
    );
    const offers = await connectMongo
      .collection("offers")
      .find({ from, to })
      .sort({ price: 1 })
      .limit(limit)
